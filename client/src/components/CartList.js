...
export default function CartList() {
    ...
    // Redirect về products nếu giỏ hàng trống
    useEffect(() => {
        if (cart.length === 0) {
    ...
    const handlePayment = () => {
        ...
        alert(`Thanh toán ${totalPrice.toLocaleString("vi-VN")} VND cho ${selectedItems.length} sản phẩm`);
    };

    if (cart.length === 0) {
        return null; // Redirect sẽ tự chạy
    }
